...
from models import create_conv_segmentation_model7 
...
def predict_frames_vieritz_by_saved_weights(width, height):
    model = create_conv_segmentation_model7(width, height)
    model.load_weights("checkpoints/segmentation_model7_slrm_average_epoch_00200.h5")
    m = np.load("images/vieritz.npy")    
    matrix = smallsize_matrix_general(m, 2)[0] 
    #p, q, r, s, t = pp.get_gradients(matrix)
    matrix = pp.slrm_cv2_average(matrix, (41, 41))  
    label_matrix = np.zeros(matrix.shape)
    for i in range(0, matrix.shape[0] - width + 1, 12): 
        for j in range(0, matrix.shape[1] - height + 1, 12): 
            #print(i, j)
    ...
    plt.imshow(label_matrix, cmap="Greys") 
    plt.show()
    np.save("images/prediction_frame_vieritz_slrm_av.npy", label_matrix)
...
def train():
    ...
    for i in range(len(x_train)):      
        x_train[i] = pp.slrm_cv2_average(x_train[i], (41, 41)) 



    for i in range(len(x_test)): 
        x_test[i] = pp.slrm_cv2_average(x_test[i], (41, 41)) 
    ...
    checkpoint_path = "checkpoints/segmentation_model7_slrm_average_epoch_{epoch:05d}.h5"
