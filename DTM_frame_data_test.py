...
def get_DTM_frame_data(bbox_manipulation=2, min_width=8, outlier_detection=False):
    ...
    dtm_list = [dtm1, dtm2, dtm3, dtm4, dtm5, dtm6, dtm7, 
                dtm8, dtm9, dtm10, dtm11, dtm12, dtm13, dtm14, 
                dtm15, dtm16, dtm17, dtm18, dtm19, dtm20, dtm21]


    """
    #dtm1 = dtm20
    dtm1.get_bboxes()
    dtm1.set_matrix(dtm1.resolution) 
    dtm1.set_label_matrix(bbox_manipulation=2)
    dtm1.create_frame_segmentation_data(kernel_size=64, augmentation=True, augmentation_steps=2, frame_steps=10)

    dtm1.show_segmentation_data()
    sys.exit()
    """
    """
    dtm_list = DTMList(dtm_list)
    dtm_list.get_frame_dataset(kernel_size=64, augmentation=True, augmentation_steps=2, frame_steps=50)
    print(len(dtm_list.X), len(dtm_list.Y))
    """



    dtm_germany_train = DTMList([dtm1, dtm2]) 
    dtm_germany_test = DTMList([dtm3, dtm21])  

    dtm_norway_train = DTMList([dtm4, dtm8, dtm9, dtm12, dtm12a, dtm13, dtm14, dtm15, dtm16, dtm17, dtm18, dtm19])
    dtm_norway_test = DTMList([dtm5, dtm6, dtm7, dtm10, dtm11, dtm20])

    for dtmlist in [dtm_germany_train, dtm_germany_test]: 
        dtmlist.get_frame_dataset(  kernel_size=64, 
                                    augmentation=True, 
        ...
        print("X and Y: ", len(dtmlist.X), len(dtmlist.Y))

    for dtmlist in [dtm_norway_train, dtm_norway_test]: 
        dtmlist.get_frame_dataset(  kernel_size=64, 
                                    augmentation=True, 
...
def get_vieritz_data(bbox_manipulation=2, min_width=8): 
    ...
    return dtm.label_matrix



def get_DTM_frame_data_bigger(bbox_manipulation=2, min_width=8, outlier_detection=False, frame_steps_germany=20, frame_steps_norway=50):
    #DTM parameters: path, filename, file_format, resolution, xml_folder="", voc=False
    path = "Forschungspraktikum/Grabhügeldetektor/Schweinert/pos_1"
    filename = "DGMLASERSCAN_379617-5719970_dgm_KOPIE"
    file_format = "xyz"
    ...
    for dtmlist in [dtm_germany_train, dtm_germany_test]: 
        dtmlist.get_frame_dataset(  kernel_size=64, 
                                    augmentation=True, 
                                    augmentation_steps=2, 
                                    frame_steps=frame_steps_germany, 
                                    bbox_manipulation=bbox_manipulation, 
                                    min_width=min_width, 
                                    outlier_detection=outlier_detection  )
    ...
    for dtmlist in [dtm_norway_train, dtm_norway_test]: 
        dtmlist.get_frame_dataset(  kernel_size=64, 
                                    augmentation=True, 
                                    augmentation_steps=8, 
                                    frame_steps=frame_steps_norway, 
                                    bbox_manipulation=bbox_manipulation, 
                                    min_width=min_width, 
                                    outlier_detection=outlier_detection  )
...
#plt.imshow(get_vieritz_data(min_width=16)) 
#plt.show()
