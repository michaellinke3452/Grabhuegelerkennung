...
def get_fullsize_rcnn_data():
    ...
    dtm_norway_train = DTMList([dtm4, dtm8, dtm9, dtm12, dtm12a, dtm13, dtm14, dtm15, dtm16, dtm17, dtm18, dtm19, dtm11, dtm20, dtm7, dtm10])
    dtm_norway_test = DTMList([dtm5, dtm6])

    augmentation_steps = 2
    dtm_germany_train.create_rcnn_fullsize_dataset( 
                                            augmentation=True, 
                                            augmentation_steps=augmentation_steps, 
                                            data_type="train"  )    
    

    dtm_germany_test.create_rcnn_fullsize_dataset(  
                                            augmentation=True,  
                                            augmentation_steps=augmentation_steps,  
                                            data_type="test"   )   

    augmentation_steps = 8
    dtm_norway_train.create_rcnn_fullsize_dataset( 
                                            augmentation=True,   
                                            augmentation_steps=augmentation_steps, 
                                            data_type="train"  )   


    dtm_norway_test.create_rcnn_fullsize_dataset( 
                                            augmentation=True, 
                                            augmentation_steps=augmentation_steps, 
                                            data_type="test"   )   
